...
function allNodeIds(node: NodeRecord): string[] { return [node.id, ...node.children.flatMap(allNodeIds)]; }
function TreeNode({ node, selected, onSelect, expanded, toggle }: { node: NodeRecord; selected: string; onSelect: (id: string) => void; expanded: Set<string>; toggle: (id: string) => void }) {
  ...
  return <li ref={drop.setNodeRef} className={drop.isOver ? "drop-target" : ""}>
    <div className={`tree-row ${selected === node.id ? "selected" : ""}`} style={{ cursor: "pointer" }} onClick={() => onSelect(node.id)}>
      ...
      <span className="tree-name">{node.name}</span><span className="count" title={`direct / including descendants; ${node.verification}`}>{node.directClaimCount}/{node.recursiveClaimCount} · {node.verification}</span><span aria-label={`${node.recursiveClaimCount ? Math.round(node.verifiedClaimCount / node.recursiveClaimCount * 100) : 0}% of claims verified`} title={`${node.verifiedClaimCount} of ${node.recursiveClaimCount} claims verified`} style={{ width: "1.1rem", height: "1.1rem", flex: "0 0 1.1rem", borderRadius: "50%", background: `conic-gradient(#20824a ${node.recursiveClaimCount ? node.verifiedClaimCount / node.recursiveClaimCount * 100 : 0}%, #d4dce7 0)` }} />
...
function Detail({ project, node, refresh, setError, onSelect }: { project: Project; node: NodeRecord; refresh: (p: Project) => void; setError: (e: string) => void; onSelect: (id: string) => void }) {
  ...
  return <main className="detail">
    ...
    {node.id === project.rootNodeId && <section><h2>Test results</h2><p className="hint">Attach machine-readable JSON, JUnit/XUnit XML, MSTest TRX, TAP, Cargo test, or Go test reports from the project’s test runs.</p><label className="upload">Select test results file <input type="file" accept="application/json,.json,.jsonl,.ndjson,application/xml,application/junit+xml,text/xml,.xml,.junit,.trx,text/tap,.tap,text/plain,.txt,.log" onChange={(e) => uploadTestResults(e.target.files?.[0])} /></label><button onClick={async () => { try { refresh(await api.verify(project.name)); } catch (e) { setError((e as Error).message); } }}>Verify</button>{project.testResults.length > 0 && <ul className="children">{project.testResults.map((file) => <li key={file.id}>{file.fileName} <button className="danger" onClick={async () => { if (confirm(`Delete ${file.fileName}?`)) try { refresh(await api.deleteTestResults(project.name, node.id, file.id)); } catch (e) { setError((e as Error).message); } }}>Delete</button></li>)}</ul>}</section>}
