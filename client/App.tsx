...
function countFailedClaims(node: NodeRecord): number { return node.claims.filter((claim) => claim.verification === "failed").length + node.children.reduce((total, child) => total + countFailedClaims(child), 0); }
function TreeNode({ node, selected, onSelect, expanded, toggle }: { node: NodeRecord; selected: string; onSelect: (id: string) => void; expanded: Set<string>; toggle: (id: string) => void }) {
  ...
  const unverifiedClaimCount = Math.max(0, node.recursiveClaimCount - node.verifiedClaimCount - failedClaimCount);
  const verifiedPercentage = node.recursiveClaimCount ? node.verifiedClaimCount / node.recursiveClaimCount * 100 : 0;
  const failedPercentage = node.recursiveClaimCount ? failedClaimCount / node.recursiveClaimCount * 100 : 0;
  return <li ref={drop.setNodeRef} className={drop.isOver ? "drop-target" : ""}>
    <div className={`tree-row ${selected === node.id ? "selected" : ""}`} style={{ cursor: "pointer" }} onClick={() => onSelect(node.id)}>
      ...
      <span className="tree-name">{node.name}</span><span className="count" title={`direct / including descendants; ${node.verification}`}>{node.directClaimCount}/{node.recursiveClaimCount} · {node.verification}</span><span aria-label={`${node.verifiedClaimCount} verified, ${failedClaimCount} failed, ${unverifiedClaimCount} unverified claims`} title={`${node.verifiedClaimCount} verified · ${failedClaimCount} failed · ${unverifiedClaimCount} unverified`} style={{ width: "1.1rem", height: "1.1rem", flex: "0 0 1.1rem", borderRadius: "50%", background: `conic-gradient(#20824a 0 ${verifiedPercentage}%, #c43d4b ${verifiedPercentage}% ${verifiedPercentage + failedPercentage}%, #d4dce7 ${verifiedPercentage + failedPercentage}% 100%)` }} />
...
export function App() {
  ...
  return <div className="app"><DndContext onDragEnd={dragEnd}><PanelGroup direction="horizontal"><Panel defaultSize={30} minSize={20} className="sidebar"><div className="project-header"><strong>{project.name}</strong><div className="project-header-actions"><button onClick={() => setExpanded(new Set(allNodeIds(project.tree)))}>Expand all</button><button onClick={async () => { try { refresh(await api.verify(project.name)); } catch (e) { setError((e as Error).message); } }}>Verify</button></div></div><nav><ul><TreeNode node={project.tree} selected={selected} onSelect={setSelected} expanded={expanded} toggle={(id) => setExpanded((old) => { const next = new Set(old); next.has(id) ? next.delete(id) : next.add(id); return next; })} /></ul></nav></Panel><PanelResizeHandle className="resize" /><Panel minSize={40}>{selectedNode && <Detail key={selectedNode.id} project={project} node={selectedNode} refresh={refresh} setError={setError} onSelect={setSelected} />}</Panel></PanelGroup></DndContext>{error && <div className="toast" onClick={() => setError("")}>{error}</div>}</div>;
}
