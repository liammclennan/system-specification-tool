...
function Detail({ project, node, refresh, setError, onSelect }: { project: Project; node: NodeRecord; refresh: (p: Project) => void; setError: (e: string) => void; onSelect: (id: string) => void }) {
  ...
  return <main className="detail">
    ...
    {node.id === project.rootNodeId && <section><h2>Verification</h2><p className="hint">Test result files are read from the configured test-results path.</p><button onClick={async () => { try { refresh(await api.verify(project.name)); } catch (e) { setError((e as Error).message); } }}>Verify</button></section>}
