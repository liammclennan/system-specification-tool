...
function TreeNode({ node, selected, onSelect, expanded, toggle }: { node: NodeRecord; selected: string; onSelect: (id: string) => void; expanded: Set<string>; toggle: (id: string) => void }) {
  ...
  return <li ref={drop.setNodeRef} className={drop.isOver ? "drop-target" : ""}>
    <div className={`tree-row ${selected === node.id ? "selected" : ""}`} onClick={() => onSelect(node.id)}>
...
function Detail({ project, node, refresh, setError, onSelect }: { project: Project; node: NodeRecord; refresh: (p: Project) => void; setError: (e: string) => void; onSelect: (id: string) => void }) {
  ...
  return <main className="detail">
    ...
    {node.id === project.rootNodeId && <section><h2>Test results</h2><p className="hint">Attach machine-readable JSON reports from the project’s test runs.</p><label className="upload">Select test results file <input type="file" accept="application/json,.json" onChange={(e) => uploadTestResults(e.target.files?.[0])} /></label><button onClick={async () => { try { refresh(await api.verify(project.name)); } catch (e) { setError((e as Error).message); } }}>Verify</button>{project.testResults.length > 0 && <ul className="children">{project.testResults.map((file) => <li key={file.id}>{file.fileName} <button className="danger" onClick={async () => { if (confirm(`Delete ${file.fileName}?`)) try { refresh(await api.deleteTestResults(project.name, node.id, file.id)); } catch (e) { setError((e as Error).message); } }}>Delete</button></li>)}</ul>}</section>}
...
function ClaimEditor({ item, project, refresh, setError }: { item: NodeRecord["claims"][number]; project: string; refresh: (p: Project) => void; setError: (e: string) => void }) {
  const [text, setText] = useState(item.text); useEffect(() => setText(item.text), [item]);
