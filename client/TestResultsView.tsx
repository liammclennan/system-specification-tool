...
export function TestResultsView({ files }: { files: VerificationTestFile[] }) {
  ...
  return (
    <div className="test-result-files">
      {files.map((file) => (
        <section className="test-result-file" key={file.fileName}>
          <header>
            <h2>{file.fileName}</h2>
          ...
          {file.tests.length ? (
            <ul>
              {file.tests.map((test, index) => (
                <li className={`test-result ${test.status}`} key={`${test.name}-${index}`}>
            ...
            <p className="hint">No tests were recognized in this file.</p>
          )}
        </section>
      ))}
    </div>
