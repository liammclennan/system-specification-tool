...
try {
  resolveStartupConfiguration(projectArgument, process.env.WORKSPACE_ROOT, process.env.SYSTEM_SPECIFICATION_TOOL_PROJECT, process.cwd(), process.env.SYSTEM_SPECIFICATION_TOOL_PORT, process.env.SYSTEM_SPECIFICATION_TOOL_TEST_RESULTS);
} catch (error) {
...
const server = spawn(executable("tsx"), ["watch", join(packageRoot, "server/index.ts"), "--", ...projectArgument], { cwd: process.cwd(), stdio: "inherit", env: { ...process.env, SYSTEM_SPECIFICATION_TOOL_DEV: "true" } });
