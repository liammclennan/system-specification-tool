...
const claim = (shortId: string, verification: VerificationStatus, ignored = false): Claim => ({
  id: shortId,
  shortId,
  nodeId: "node",
  text: `${shortId} claim`,
...
const node = (name: string, claims: Claim[], children: NodeRecord[] = []): NodeRecord => ({
  id: name,
  shortId: name,
  name,
  parentId: null,
  ...
  claims,
  children,
  directClaimCount: claims.length,
  recursiveClaimCount: claims.length,
  verifiedClaimCount: 0,
  failedClaimCount: 0,
  ignoredClaimCount: 0,
  verification: "unverified",
});
const project = (tree: NodeRecord): Project => ({
  id: "system",
  name: "system",
  rootNodeId: tree.id,
  tree,
...
describe("verificationReport", () => {
  it("abb0 ddba reports outstanding claims with their node and exits with code 1", () => {
    const result = verificationReport(
      project(
        node(
          "System",
          [claim("ok", "verified"), claim("skip", "failed", true)],
          [node("Payments", [claim("bad", "failed"), claim("todo", "unverified")])],
        ),
      ),
    );
  ...
  it("ddba exits with code 0 when every claim is verified or ignored", () => {
    const result = verificationReport(
      project(node("System", [claim("ok", "verified"), claim("skip", "unverified", true)])),
    );
