...
describe("resolveStartupConfiguration", () => {
  ...
  it("uses the environment project when no command-line project is supplied 3177", () => {
    expect(resolveStartupConfiguration(["--test-results=/results"], "/repositories/from-env", "/workspace")).toMatchObject({ workspaceRoot: "/repositories", initialProject: "from-env", initialProjectPath: "/repositories/from-env", testResultsPath: "/results" });
  });
  it("resolves relative project arguments from the caller's working directory", () => {
    expect(resolveStartupConfiguration(["--project=projects/demo", "--test-results=results"], undefined, "/workspace")).toMatchObject({ workspaceRoot: "/workspace/projects", initialProject: "demo", initialProjectPath: "/workspace/projects/demo", testResultsPath: "/workspace/results" });
  });
  it("accepts a validated --port argument", () => {
    expect(resolveStartupConfiguration(["--port=4123", "--test-results=/results"], undefined, "/workspace").port).toBe(4123);
