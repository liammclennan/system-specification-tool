...
export function resolveStartupConfiguration(
  args: string[],
  environmentProject?: string,
  callerDirectory = process.cwd(),
  configuredPort?: string,
  environmentTestResults?: string,
): StartupConfiguration {
  const portArgument =
    args.find((arg) => arg.startsWith("--port="))?.slice("--port=".length) ??
    (() => {
  ...
  for (const option of ["--project", "--test-results", "--port"]) {
    const index = args.indexOf(option);
    if (index >= 0 && args[index + 1]) optionValues.add(args[index + 1]);
  }
  ...
  const testResultsArgument =
    args.find((arg) => arg.startsWith("--test-results="))?.slice("--test-results=".length) ??
    (() => {
      const index = args.indexOf("--test-results");
      return index >= 0 ? args[index + 1] : undefined;
    })() ??
    environmentTestResults;
  const absoluteProjectPath = resolve(callerDirectory, projectPath);
  return {
    workspaceRoot: dirname(absoluteProjectPath),
    initialProject: basename(absoluteProjectPath),
    initialProjectPath: absoluteProjectPath,
    port,
    testResultsPath: testResultsArgument
      ? resolve(callerDirectory, testResultsArgument)
      : undefined,
  };
}
