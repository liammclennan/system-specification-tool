...
describe("ProjectStorage", () => {
  it("f735 assigns deterministic unique node short IDs and extends colliding hashes", async () => {
    const collision = new Map<string, string>(); let pair: [string, string] | undefined;
    for (let index = 0; !pair; index++) {
      const id = `node-${index}`; const prefix = createHash("sha256").update(id).digest("hex").slice(0, 4);
      const prior = collision.get(prefix); if (prior) pair = [prior, id]; else collision.set(prefix, id);
    }
    ...
    expect(project.tree.shortId).toBe(createHash("sha256").update(project.tree.id).digest("hex").slice(0, 4));
  });
  it("2116 assigns claim short IDs using the same uniqueness rules as nodes", async () => {
    ...
    expect(claim.shortId).toBe(createHash("sha256").update(claim.id).digest("hex").slice(0, claim.shortId.length));
  ...
  it("45b1 570d persists a hierarchy and permits claims on nested named nodes", async () => {
    ...
    const loaded = await store.openProject("payment-system"); expect(loaded.tree.recursiveClaimCount).toBe(2); expect(loaded.tree.children[0].name).toBe("Gateway"); expect(loaded.tree.children[0].directClaimCount).toBe(1); expect(loaded.tree.children[0].shortId).toHaveLength(4);
  });
  it("227d prevents moving the root or a node into a descendant", async () => {
    ...
    const child = project.tree.children[0]; project = await store.createNode("system", project.rootNodeId, "Destination"); const destination = project.tree.children.find((node) => node.name === "Destination")!;
    await expect(store.moveNode("system", project.rootNodeId, child.id)).rejects.toBeInstanceOf(StorageError);
  ...
  it("ba1f stores non-claim Markdown text and uploaded images on a node", async () => {
    const store = await fixture(); let project = await store.createProject("system");
    project = await store.updateNode("system", project.rootNodeId, { content: "Architecture notes." });
    expect(project.tree.content).toBe("Architecture notes.");
    const reference = await store.saveAsset("system", project.rootNodeId, { buffer: Buffer.from("image"), mimetype: "image/png", originalname: "diagram.png" } as Express.Multer.File);
    expect(reference).toMatch(/^\.\.\/assets\/.+\.png$/);
    expect(await readFile(join(roots[0], "system", reference.replace("../", "")), "utf8")).toBe("image");
  });
