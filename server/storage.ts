...
function parseTapAssertions(tap: string): TestAssertion[] {
  ...
  for (const line of tap.split(/\r?\n/)) {
    ...
    assertions.push({
      name,
      status: directive ? "skipped" : match[1].toLowerCase() === "ok" ? "passed" : "failed",
    });
  }
  return assertions;
}
...
export class ProjectStorage {
  ...
  private safeProject(name: string) {
    if (!/^[a-zA-Z0-9][a-zA-Z0-9._ -]*$/.test(name) || name.includes(".."))
      throw new StorageError("Invalid project name");
    const path = resolve(this.root, name);
    if (relative(this.root, path).startsWith(".."))
      throw new StorageError("Project is outside workspace");
    return path;
  }
  ...
  async createProject(name: string): Promise<Project> {
    ...
    try {
      ...
      throw new StorageError("A project with that name already exists");
    } catch (error) {
  ...
  private async initializeProject(projectPath: string, name: string) {
    ...
    await this.atomic(
      join(projectPath, "specification.json"),
      JSON.stringify({ formatVersion: 1, rootNodeId: id }, null, 2),
    );
  ...
  private async readManifest(path: string): Promise<Manifest> {
    try {
      const manifest = JSON.parse(
        await readFile(join(path, "specification.json"), "utf8"),
      ) as Manifest;
      if (manifest.formatVersion !== 1 || !manifest.rootNodeId) throw new Error();
      return manifest;
    } catch {
      throw new StorageError("Project has an invalid specification.json");
    }
  }
  private async writeNode(projectPath: string, node: NodeMeta) {
    ...
    try {
      ...
      await this.atomic(contentPath, "");
    }
  }
  private claimFile(
    claim: Pick<StoredClaim, "id" | "nodeId" | "text" | "order"> & {
  ...
  private specificationMarkdown(project: Project, createdAt: Date, specificationPath: string) {
    ...
    const renderNode = (node: NodeRecord, depth: number): string => {
      const claims = node.claims
        .map(
          (claim) =>
            `- **${claim.ignored ? "ignored" : claim.verification}** — [${claim.shortId}] ${claim.text.replace(/\n/g, "\n  ")}`,
        )
        .join("\n");
  ...
  private async load(projectPath: string) {
    ...
    const nodeFiles = (await readdir(join(projectPath, "nodes"))).filter((file) =>
      file.endsWith(".json"),
    );
    ...
    for (const file of (await readdir(join(projectPath, "claims"))).filter((entry) =>
      ...
      claims.push({
        id: match[1],
        nodeId: match[2],
        order: Number(match[3] ?? Number.MAX_SAFE_INTEGER),
        verification: (match[4] ?? "unverified") as VerificationStatus,
        ignored: match[5] === "true",
        text: match[6].trim(),
        shortId: "",
      });
    }
    ...
    claims.forEach((claim) => {
      claim.shortId = this.short(claim.id, used);
    });
  ...
  async openProject(name: string): Promise<Project> {
    ...
    for (const node of nodes.values()) {
      if (node.parentId && !nodes.has(node.parentId))
        throw new StorageError(`Node ${node.name} has a missing parent`);
      if (node.parentId)
    ...
    const build = async (node: NodeMeta, trail = new Set<string>()): Promise<NodeRecord> => {
      ...
      const nodeClaims = claims
        .filter((claim) => claim.nodeId === node.id)
        .sort((a, b) => a.order - b.order || a.id.localeCompare(b.id));
      const nested = await Promise.all(
        (children.get(node.id) ?? [])
          .sort((a, b) => a.name.localeCompare(b.name))
          .map((child) => build(child, nextTrail)),
      );
      const directClaimCount = nodeClaims.length;
      const verifiedClaimCount =
        nodeClaims.filter((claim) => claim.verification === "verified").length +
        nested.reduce((sum, child) => sum + child.verifiedClaimCount, 0);
      const failedClaimCount =
        nodeClaims.filter((claim) => claim.verification === "failed").length +
        nested.reduce((sum, child) => sum + child.failedClaimCount, 0);
      const ignoredClaimCount =
        nodeClaims.filter((claim) => claim.ignored).length +
        nested.reduce((sum, child) => sum + child.ignoredClaimCount, 0);
      const includedClaims = nodeClaims.filter((claim) => !claim.ignored);
      const verification: VerificationStatus =
        includedClaims.some((claim) => claim.verification === "failed") ||
        nested.some((child) => child.verification === "failed")
          ? "failed"
          : includedClaims.every((claim) => claim.verification === "verified") &&
              nested.every((child) => child.verification === "verified")
            ? "verified"
            : "unverified";
      return {
        id: node.id,
        shortId: nodeShortIds.get(node.id)!,
        name: node.name,
        parentId: node.parentId,
        content: await readFile(join(path, "nodes", `${node.id}.md`), "utf8"),
        claims: nodeClaims,
        children: nested,
        directClaimCount,
        recursiveClaimCount:
          directClaimCount + nested.reduce((sum, child) => sum + child.recursiveClaimCount, 0),
        verifiedClaimCount,
        failedClaimCount,
        ignoredClaimCount,
        verification,
    ...
    try {
      testResults = (await readdir(join(path, "test-results"))).flatMap((file) => {
        const match = file.match(/^([0-9a-f-]{36})__(.+)$/);
        return match ? [{ id: match[1], fileName: match[2] }] : [];
      });
    } catch {
      /* optional legacy directory */
    }
    return {
      id: basename(path),
      name: basename(path),
      rootNodeId: manifest.rootNodeId,
      tree: await build(nodes.get(manifest.rootNodeId)!),
      testResults,
    };
  }
  async createNode(project: string, parentId: string, name: string) {
    ...
    await this.writeNode(path, node);
  ...
  async updateNode(project: string, id: string, changes: { name?: string; content?: string }) {
    ...
    if (changes.name !== undefined) {
      if (!changes.name.trim()) throw new StorageError("Node name is required");
      await this.writeNode(path, { ...node, name: changes.name.trim() });
    }
    if (changes.content !== undefined)
      await this.atomic(join(path, "nodes", `${id}.md`), changes.content);
    return this.openProject(project);
  }
  async moveNode(project: string, id: string, parentId: string) {
    ...
    while (cursor) {
      if (cursor === id) throw new StorageError("A node cannot be moved into its own descendant");
      cursor = loaded.nodes.get(cursor)?.parentId ?? null;
    }
    await this.writeNode(path, { ...node, parentId });
    return this.openProject(project);
  }
  async deleteNode(project: string, id: string) {
    ...
    const node = loaded.nodes.get(id);
    if (!node) throw new StorageError("Node was not found");
    if (id === loaded.manifest.rootNodeId || !node.parentId)
      throw new StorageError("The root node cannot be deleted");
    const parentId = node.parentId;
    let order =
      Math.max(
        -1,
        ...loaded.claims.filter((claim) => claim.nodeId === parentId).map((claim) => claim.order),
      ) + 1;
    for (const claim of loaded.claims
      .filter((item) => item.nodeId === id)
      .sort((a, b) => a.order - b.order)) {
      await this.atomic(
        join(path, "claims", `${claim.id}.md`),
        this.claimFile({ ...claim, nodeId: parentId, order: order++ }),
      );
    }
  ...
  async createClaim(project: string, nodeId: string, text: string) {
    ...
    if (!text.trim()) throw new StorageError("Claim text is required");
    const id = randomUUID();
    const order =
      Math.max(
        -1,
        ...loaded.claims.filter((claim) => claim.nodeId === nodeId).map((claim) => claim.order),
      ) + 1;
    await this.atomic(
      join(path, "claims", `${id}.md`),
      this.claimFile({ id, nodeId, order, text }),
    );
    return this.openProject(project);
  }
  async updateClaim(project: string, id: string, text: string) {
    const path = this.safeProject(project);
    const claim = (await this.load(path)).claims.find((item) => item.id === id);
    if (!claim) throw new StorageError("Claim was not found");
    if (!text.trim()) throw new StorageError("Claim text is required");
    await this.atomic(join(path, "claims", `${id}.md`), this.claimFile({ ...claim, text }));
    return this.openProject(project);
  }
  async setClaimIgnored(project: string, id: string, ignored: boolean) {
    ...
    await this.atomic(
      ...
      this.claimFile({
        ...claim,
        ignored,
        verification: ignored ? "unverified" : claim.verification,
      }),
    );
    return this.openProject(project);
  }
  async moveClaim(project: string, id: string, nodeId: string) {
    ...
    const order =
      Math.max(
        -1,
        ...loaded.claims.filter((item) => item.nodeId === nodeId).map((item) => item.order),
      ) + 1;
    await this.atomic(
      join(path, "claims", `${id}.md`),
      this.claimFile({ ...claim, nodeId, order }),
    );
    return this.openProject(project);
  }
  async reorderClaims(project: string, nodeId: string, orderedIds: string[]) {
    ...
    if (
      claims.length !== orderedIds.length ||
      new Set(orderedIds).size !== orderedIds.length ||
      !orderedIds.every((id) => claims.some((claim) => claim.id === id))
    )
      throw new StorageError("Claim order does not match this node");
    await Promise.all(
  ...
  async deleteClaim(project: string, id: string) {
    const path = this.safeProject(project);
    const file = join(path, "claims", `${id}.md`);
    try {
      await unlink(file);
    } catch {
      throw new StorageError("Claim was not found");
    }
    return this.openProject(project);
  }
  async saveAsset(project: string, nodeId: string, file: Express.Multer.File) {
    const path = this.safeProject(project);
    if (!(await this.load(path)).nodes.has(nodeId)) throw new StorageError("Node was not found");
    if (!file.mimetype.startsWith("image/"))
      throw new StorageError("Only image uploads are supported");
    const extension = extname(file.originalname).replace(/[^.a-zA-Z0-9]/g, "") || ".img";
    const id = randomUUID();
  ...
  async verify(project: string, resultsPath = join(this.safeProject(project), "test-results")) {
    ...
    await Promise.all(
      loaded.claims.map((claim) => {
        ...
        const verification: VerificationStatus = matches.some(
          (assertion) => assertion.status === "failed",
        )
          ? "failed"
          : matches.some((assertion) => assertion.status === "passed")
            ? "verified"
            : "unverified";
        return this.atomic(
          join(path, "claims", `${claim.id}.md`),
          this.claimFile({ ...claim, verification }),
        );
      }),
    );
    ...
    await this.atomic(
      specificationPath,
      this.specificationMarkdown(verifiedProject, new Date(), specificationPath),
    );
